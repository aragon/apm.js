...
module.exports = (web3, options = {}) => {
  ...
  return {
    ...
    /**
     * Upload files under `directory` to storage provider `provider`.
     *
     * Returns a content URI string.
     *
    ...
    async uploadFilesToStorageProvider(provider, directory) {
      return providers[provider].uploadFiles(directory)
    },

    /**
     * Publishes a new version (`version`) of `appId` with content URI `contentURI`.
     *
     ...
     * @param {string} contentURI The URI of the content to be published.
     ...
     */
    async publishVersion(manager, appId, version, contentURI, contract, from) {
     ...
     */
    async publishVersionIntent(manager, appId, version, contentURI, contract) {
      if (!semver.valid(version)) {
        throw new Error(`${version} is not a valid semantic version`)
      }

      // convert content URI to string hex
      const hexContentURI = Buffer.from(contentURI).toString('hex')
      ...
      if (repo !== null) {
        return {
          ...
          params: [formatVersion(version), contract, `0x${hexContentURI}`],
          targetContract: repo,
        }
      } else {
